  ...
  const handleEmailSignIn = async (e) => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOtp({
        email: email.trim(),
        options: { emailRedirectTo: `${window.location.origin}/auth/callback?next=/dashboard_overview` },
      });
          ...
          <div className="pt-4 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-3 max-w-md mx-auto">
              ...
              <button 
                ...
                className="flex items-center justify-center gap-3 px-6 py-3 bg-[#1e1f26] hover:bg-[#282a30] border border-[#424754]/20 rounded-xl transition-all group w-full sm:w-auto"
              >
              ...
              {/* Email Login */}
              {emailSent ? (
                <div className="flex items-center justify-center gap-2 px-6 py-3 bg-[#a8edea]/10 border border-[#a8edea]/20 rounded-xl w-full sm:w-auto">
                  <span className="material-symbols-outlined text-[#a8edea] text-xl">check_circle</span>
                  <span className="text-sm font-medium text-[#a8edea]">Magic link sent! Check your inbox.</span>
                </div>
              ) : showEmailInput ? (
                <form onSubmit={handleEmailSignIn} className="flex items-center gap-2 w-full sm:w-auto">
                  <input
                    type="email"
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    placeholder="your@email.com"
                    className="bg-[#0c0e14] border border-[#424754]/20 rounded-xl px-4 py-3 text-sm text-[#e2e2eb] placeholder:text-[#8c909f]/60 focus:outline-none focus:border-[#adc6ff]/40 w-full"
                    autoFocus
                  />
                  <button type="submit" className="bg-gradient-to-br from-[#adc6ff] to-[#4d8eff] text-[#00285d] px-4 py-3 rounded-xl text-sm font-bold hover:opacity-90 transition-all whitespace-nowrap">
                    Send Link
                  </button>
                </form>
              ) : (
